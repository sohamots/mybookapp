...
@IonicPage()
@Component({
  selector: 'page-addaccounts',
  templateUrl: 'addaccounts.html',
})
export class AddaccountsPage {
  ...
  save1() {
    ...
    this.auth.add_bank_accounts(data).then((res) => {
        if (res["status"] == "1") {
  ...
  save2() {
    ...
    this.auth.add_fixed_deposits(data).then((res) => {
        if (res["status"] == "1") {
            let alert = this.alert.create({
                title: 'Success',
                subTitle: res["message"],
  ...
  save3() {
    ...
    this.auth.add_demat_accounts(data).then((res) => {
        if (res["status"] == "1") {
            let alert = this.alert.create({
                title: 'Success',
                subTitle: res["message"],
  ...
  save4() {
    ...
    this.auth.add_credit_cards(data).then((res) => {
        if (res["status"] == "1") {
            let alert = this.alert.create({
                title: 'Success',
                subTitle: res["message"],
  ...
  save5() {
    ...
    this.auth.add_lockers(data).then((res) => {
        if (res["status"] == "1") {
            let alert = this.alert.create({
                title: 'Success',
                subTitle: res["message"],
